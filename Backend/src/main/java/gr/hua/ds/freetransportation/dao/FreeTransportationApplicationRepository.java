...
@Repository
public interface FreeTransportationApplicationRepository extends CrudRepository<FreeTransportationApplication, Integer> {
    ...
    public List<FreeTransportationApplication> findUnansweredApplications();

    @Query("SELECT a FROM FreeTransportationApplication a WHERE a.status = 'PENDING' AND a.validated = true")
    public List<FreeTransportationApplication> findUnansweredAndValidatedApplications();
}
